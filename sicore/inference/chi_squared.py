...
class InferenceChiSquared(ABC):
    ...
    def __init__(self, data, var, eta, degree):
        self.data = data
        self.eta = eta
        self.var = var
        self.length = len(data)
        self.N = int(self.length/degree)
        self.degree = degree
        ...
        if isinstance(self.var, (int, float)):
            self.cov = self.var * np.identity(self.N)
        else:
            self.cov = np.asarray(self.var)
        self.sigma_eta = np.dot(self.cov, eta)
        self.eta_sigma_eta = np.dot(eta.T, self.sigma_eta)
        self.sigma_hat = self.eta_sigma_eta/np.dot(eta.T, eta)

        P = np.kron(np.eye(self.degree), np.dot(eta, eta.T)/np.dot(eta.T, eta))
        self.P_data = np.dot(P, data)
        # P_vecx = eta_data.T.flatten()
        self.stat = np.linalg.norm(self.P_data, ord=2)/math.sqrt(self.sigma_hat[0][0])
        ##################################################################
...
class NaiveInferenceChiSquared(InferenceChiSquared):
    ...
    def test(self, tail="double", popmean=0):
        ...
        # TODO: check perform this function
...
class SelectiveInferenceChiSquared(InferenceChiSquared):
    ...
    def test(
        ...
        # stat = standardize(self.stat, popmean, self.eta_sigma_eta)
        # chi_intervals = standardize(intervals, popmean, self.eta_sigma_eta)
        # TODO: check perform this function
        chi = tc2_cdf_mpmath(self.stat**2, np.power(intervals, 2), self.degree, dps=dps, out_log=out_log)

        return calc_pvalue(chi, tail=tail)


class SelectiveInferenceChiSquaredSE(SelectiveInferenceChiSquared):
    """
    ...
    def __init__(self, data, var, eta, degree, init_lower=NINF, init_upper=INF):
        super().__init__(data, var, eta, degree)
